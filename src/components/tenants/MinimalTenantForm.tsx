...
interface MinimalTenantFormData {
  fullName: string;
  phone: string;
  email?: string;
  occupation: string;
...
interface MinimalTenantFormProps {
...
export function MinimalTenantForm({
  onSubmit,
  onCancel,
  isOpen,
  title = "Add Tenant",
  tenant,
}: MinimalTenantFormProps) {
  ...
  const onFormSubmit = async (data: MinimalTenantFormData) => {
    try {
      setIsSubmitting(true);

      // Create tenant object with minimal required fields
      const tenantData: Tenant = {
        id: tenant?.id || `tenant_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        personalInfo: {
